...
import { MapPin, Mail, Briefcase, Calendar, Building, User, GraduationCap, Phone, Trash2, Loader2 } from "lucide-react";
import { Linkedin } from "lucide-react";
import { Student, getDirectImageUrl, deleteStudentData } from "@/services/apiService";
...
const StudentGrid = ({ students, isLoggedIn, loading = false, currentUserInfo, userRole, onStudentDeleted }: StudentGridProps) => {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      toast({ title: "Deleted", description: `${deleteTarget.name} has been removed from the directory.` });
      if (onStudentDeleted) onStudentDeleted(deleteTarget.id);
    } catch (err) {
...
My Details:
Name: ${currentUserInfo?.name || 'Fellow Alumni'}
Registration No: ${currentUserInfo?.registrationNo || 'N/A'}`;
    if (currentUserInfo?.currentJob && currentUserInfo.currentJob !== "Not specified" && currentUserInfo.currentJob !== "NA") {
...
Best regards,
${currentUserInfo?.name || 'Fellow Alumni'}
          ...
          >
            ...
            <CardContent className="p-5 -mt-4 relative z-10">
              ...
              <div className="flex gap-5 mb-5">
                ...
                <div className="flex-shrink-0 relative">
                  <Link href={`/alumni/${student.registrationNo}`} onClick={(e) => e.stopPropagation()}>
                    <div style={{borderColor: `hsl(var(--${schoolColor}))`}} className="border-3 border-white shadow-elegant rounded-xl ring-2 ring-primary/20 cursor-pointer">
                      <RobustImage
                ...
                <div className="flex-1 min-w-0 space-y-3">
                  ...
                  <div className="pl-1">
                    <Link href={`/alumni/${student.registrationNo}`} onClick={(e) => e.stopPropagation()}>
                      <h3 className="font-bold text-foreground text-lg leading-tight mb-1 text-gradient-primary hover:underline cursor-pointer">
                        {student.name}
                      </h3>
                    </Link>
                  </div>
                  ...
                  {/* Registration No */}
                  <div className="pl-1">
                    <span className="text-sm text-primary font-mono font-bold bg-primary/10 px-2.5 py-1 rounded-lg border border-primary/10 inline-block">
                ...
                <div className="flex gap-3 pt-4 border-t border-primary/20">
                  ...
                  {isStrictAdmin && (
                    <Button
                      variant="outline"
                      size="sm"
                      className="h-11 px-3 text-sm font-bold bg-gradient-to-r from-destructive to-destructive/80 text-white border-0 rounded-xl shadow-glow hover:scale-105 transition-all duration-300 relative z-20"
      ...
      <Dialog open={!!deleteTarget} onOpenChange={(open) => { if (!open) setDeleteTarget(null); }}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to permanently delete <strong>{deleteTarget?.name}</strong> ({deleteTarget?.registrationNo}) from the directory? This action cannot be undone.
            </DialogDescription>
          </DialogHeader>
